        ...
        [
            ...
            (
                ...
                RandomForestClassifier(
                    class_weight="balanced", random_state=request.param
                ),
            ),
        ]
    )
...
def test_dump_model(fitted_model, get_dummy_data, request, tmpdir):
    ...
    with open(os.path.join(tmpdir, "final_model.dill"), "rb") as input_file_stream:
        fitted_model_reload = dill.load(input_file_stream)
    assert id(fitted_model) != id(fitted_model_reload)
    assert (
        fitted_model.steps[0][1].vocabulary_
        == fitted_model_reload.steps[0][1].vocabulary_
    )
    assert (fitted_model.steps[0][1].idf_ == fitted_model_reload.steps[0][1].idf_).all()
    assert (
        fitted_model.predict_proba(new_data.X)
        == fitted_model_reload.predict_proba(new_data.X)
    ).all()
